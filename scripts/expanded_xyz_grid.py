...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        #-----------NEW--------------
        #----------------------------
        with gr.Accordion("Parameters info", open=False):
            gr.Markdown("Multitool example: Steps: 10, 20 | Sampler: Euler a, DPM++ SDE Karras | cfg scale: 3, 7 -> you have to separate parameters using |. You need to write the exact parameter's name, which are: \"seed, var. seed, var. strenght, steps, cfg scale, prompt s/r, prompt order, sampler, checkpoint name, Negative Guidance minimum sigma, sigma churn, sigma min, sigma max, sigma noise, Schedule type, Schedule min sigma, Schedule max sigma, Schedule rho, eta, clip skip, denoising, Initial noise multiplier, Extra noise, Hires upscaler, Cond. Image Mask Weight, VAE, Styles, UniPC Order, Face restore, Token merging ratio, Token merging ratio high-res, Always discard next-to-last sigma, SGM noise multiplier, Refiner checkpoint, Refiner switch at, RNG source, FP8 mode, Size, prompt s/r placeholder, prompt matrix, multitool\". If you get \"AttributeError: 'NoneType' object has no attribute 'label'\" this means you wrote one of them wrong.")
            gr.Markdown("About Prompt S/R Placeholder: the default prompt will be generated without the placeholder value. For now you can't use words inside brackets, but the placeholder in the default prompt can.")
            gr.Markdown("About Prompt matrix: the script will replace the placeholder value with different combinations of other words (the ones chosen below). As before, no brackets, except the placeholder in the default prompt.")
        #----------------------------
        #-----------END--------------
        #----------------------------
        with gr.Row():
    ...
    def run(self, p, x_type, x_values, x_values_dropdown, y_type, y_values, y_values_dropdown, z_type, z_values, z_values_dropdown, draw_legend, include_lone_images, include_sub_grids, no_fixed_seeds, vary_seeds_x, vary_seeds_y, vary_seeds_z, margin_size, csv_mode):
        ...
        if z_opt.label == "Multitool":
            zs = parse_multitool(z_values)
        else:
            zs = process_axis(z_opt, z_values, z_values_dropdown)
        if z_opt.confirm:
            z_opt.confirm(p, zs)
        ...
        print(f"Expanded X/Y/Z plot will create {len(xs) * len(ys) * len(zs) * image_cell_count} images on {len(zs)} {len(xs)}x{len(ys)} grid{plural_s}{cell_console_text}. (Total steps to process: {total_steps})")
